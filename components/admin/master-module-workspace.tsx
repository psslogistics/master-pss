...
export default function MasterModuleWorkspace({ module }: Props) {
  ...
  function submit() {
    ...
    if (href === "/operations/bookings") {
      ...
      mutateAdminState("Created client onboarding record", (current) => ({ ...current, clients: [{ id, code: `CL-${String(Date.now()).slice(-4)}`, name: form.name || "New client", city: form.city || "Pending verification", status: "On hold", onboardedByEmployeeId: "emp-admin", assignedToEmployeeId: form.assignee || employees[0]?.id || "emp-admin", shipmentVolume: 0, openTickets: 0, lastActivity: "Just now" }, ...current.clients] }), { entityType: "Client", entityId: id, entityLabel: form.name || "New client", severity: "Important" });
      setNotice(`${form.name || "Client"} onboarding record staged locally.`);
