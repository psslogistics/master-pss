...
export default function MasterModuleWorkspace({ module }: Props) {
  ...
  useEffect(() => {
    ...
    void source
      .then((result) => { if (!active) return; setLiveRows(result as Row[]); })
      .catch(() => { if (active) setLiveRows([]); });
    return () => { active = false; };
  ...
  const readOnly = !hasSupportedWorkspaceWrite(module.href) && !/export|refresh|check|mark all read/i.test(meta.primary);
...
function DetailPanel({ module, row, onAction }: { module: Props["module"]; row?: Row; onAction(action: string, id: string): void }) {
  ...
  const canAct = isPickup || module.href === "/operations/exceptions" || module.href === "/tasks" || module.href === "/support/tickets" || module.href === "/support/sla-escalations" || module.href === "/finance/wallets" || module.href === "/finance/billing-invoices" || module.href === "/system/security-sessions" || module.href.includes("scheduled-jobs") || (module.href === "/integrations/api-webhooks" && !row.id.startsWith("webhook-") && !row.id.startsWith("integration-")) || (module.href === "/integrations/couriers" && row.id.startsWith("account-"));
